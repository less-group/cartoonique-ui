...
class ImageProcessingManager {
  constructor() {
    ...
    this.processnonwaterimgbase64 = null;
  ...
  setupEventListeners() {
    ...
    document.addEventListener("pixar-transform-complete", (event) => {
      console.log(
        "Received pixar-transform-complete event with data:",
        event.detail
      );
    ...
    document.addEventListener("pixar-final-processing-complete", (event) => {
      console.log(
        "Received pixar-final-processing-complete event with data:",
        event.detail
      );
  ...
  findAndSetupPixarComponent() {
    ...
    if (!this.pixarComponent) {
      ...
      if (window.pixarComponentRetryCount >= 5) {
        console.log(
          "Creating fallback pixar component after multiple retry attempts"
        );
  ...
  setupPixarComponent(component) {
    ...
    document.dispatchEvent(
      new CustomEvent("pixar-component-setup-complete", {
      ...
      this.sendImageToRailway(this.originalFile, { isOriginal: true })
        .then((result) => {
          if (result.alreadyProcessing) {
            console.log(
              "🖼️ File is already being processed, no need to send again"
            );
          } else {
  ...
  showImageCropper() {
    ...
    console.log(
      "Loading image into cropper, data URL length:",
      this.originalImageDataUrl.length
    );
    ...
    const processImage = () => {
      ...
      if (document.readyState !== "complete") {
        console.log(
          "Document not ready yet, waiting before initializing cropper..."
        );
      ...
      this.imageCropper
        .loadImage(this.originalImageDataUrl)
        .then(() => {
        ...
        .catch((error) => {
          ...
          if (this.originalFile) {
            console.log(
              "Trying fallback: loading file directly instead of data URL"
            );
            this.imageCropper
              .loadImage(this.originalFile)
              .then(() => console.log("Fallback image loading successful"))
              .catch((err) =>
                console.error("Fallback image loading also failed:", err)
              );
          }
        });
    };
  ...
  handleCropApplied(event) {
    ...
    if (event.detail && event.detail.cropCoordinates) {
      ...
      console.log(
        "Crop coordinates saved with original dimensions:",
        JSON.stringify(this.cropCoordinates)
      );
    } else {
    ...
    if (event.detail && event.detail.croppedImage) {
      ...
      console.log(
        "Cropped image preview saved, length:",
        this.croppedImageDataUrl ? this.croppedImageDataUrl.length : 0
      );
    }
    ...
    setTimeout(() => {
      ...
      if (this.transformationComplete && this.textProcessingComplete) {
        console.log(
          "Railway image already received, applying crop immediately"
        );
    ...
    if (window.pixarTextManager || window.PixarTextManager) {
      ...
      if (textManager) {
        try {
          ...
          const selectedRatio = this.imageCropper
            ? this.imageCropper.options.aspectRatio
            : 3 / 4;
          const is5070Selected = Math.abs(selectedRatio - 5 / 7) < 0.01; // Check if close to 5/7 ratio (50x70)

          console.log(
            `Using crop ratio: ${selectedRatio}, 50x70 selected: ${is5070Selected}`
          );
          ...
          textManager.showTextDialog(file).then((completed) => {
            ...
            if (completed) {
              ...
              if (textManager.names.name1 || textManager.names.name2) {
                ...
                const fontFamily =
                  textManager.fontFamily || "Arial, sans-serif";
                ...
                const croppedWidth =
                  croppedImg.width || this.cropCoordinates.width;
                const croppedHeight =
                  croppedImg.height || this.cropCoordinates.height;
                ...
                console.log(
                  "TEXT OVERLAY: Text information captured:",
                  this.textFields
                );
                console.log(
                  "TEXT OVERLAY: Cropped dimensions for text:",
                  croppedWidth,
                  "x",
                  croppedHeight
                );
    ...
    textManager.showTextDialog = function (file) {
      ...
      return originalShowTextDialog.call(this, file).then((result) => {
        ...
        if (dialogContainer) {
          ...
          const buttonsRow = dialogContainer.querySelector(
            ".pixar-text-buttons"
          );
          ...
          if (!dialogContainer.querySelector("#pixar-subtitle-input")) {
            ...
            subtitleInput.placeholder =
              "based on a true story - your text here";
            ...
            const previewSubtitle = dialogContainer.querySelector(
              ".pixar-subtitle-overlay"
            );
            if (!previewSubtitle && this.names.subtitle) {
              // If subtitle overlay doesn't exist but we have subtitle text, create it
              const imagePreview = dialogContainer.querySelector(
                ".pixar-preview-image"
              );
              if (imagePreview) {
  ...
  applyFinalProcessing() {
    ...
    try {
      ...
      if (this.stylizedImageUrl) {
        ...
        console.log(
          "FINAL PROCESSING: Using CSS-based cropping for Railway image"
        );
        console.log(
          "FINAL PROCESSING: Text info stored:",
          this.textInfo ? "Yes" : "No"
        );
      ...
      console.error(
        "FINAL PROCESSING: Error applying final processing:",
        error
      );
  ...
  applyImageProcessing(imageUrl) {
    return new Promise((resolve, reject) => {
      try {
        // alert("applyImageProcessing 1193 called" );
        console.log("🔄 Applying cropping and text overlay to:", imageUrl);
        ...
        img.onload = () => {
          ...
          // const croppedCanvas = this.getCroppedImageFromDOM(img);
          // alert("img.onload 1206");
          if (!this.cropCoordinates || !this.originalImageWidth) {
            ...
            ctx.drawImage(
              img,
              0,
              0, // Destination X, Y (always 0, 0 for our canvas)
          ...
          // Now add the text
          console.log("SD this.textInfo" + this.textInfo);
          if (this.textInfo) {
            this.drawTextOnCanvas(ctx, canvas.width, canvas.height);
          }

          // Convert canvas to data URL
          this.nonWatermarkedCanvas = canvas;
          const processedUrl = canvas.toDataURL("image/png");

          if (
            window.resultPopupManager.selectedSizewatermark &&
            window.resultPopupManager.selectedSizewatermark == "applyonce"
          ) {
            window.resultPopupManager.applyAspectRatioToResultImage(
              window.resultPopupManager.selectedSize
            );
          }
  ...
  async hideLoadingPopup() {
    // alert("hideLoadingPopup -> " + this.stylizednonImageUrl);
    console.log(
      "this.processnonwaterimgstatus 1 : -> " + this.processnonwaterimgstatus
    ); //sd 1

    // our code will go here
    if (!this.processnonwaterimgstatus) {
      this.processnonwaterimgstatus = "processing";
      if (!this.stylizednonImageUrl) {
        this.processnonwaterimgstatus = "error";
      } else {
        try {
          this.finalNonWatermarkedImageUrl = await this.applyImageProcessing(
            this.stylizednonImageUrl
          );
          console.log(
            "✅ Processed non-watermarked image:",
    ...
    if (loadingPopup && loadingPopup.style.display !== "none") {
      ...
      if (this.finalProcessingComplete) {
        console.log(
          "🖼️ Forcing loading popup to close because final processing is complete"
        );
        loadingPopup.style.display = "none";
        ...
        console.log(
          "⚠️ Loading popup hide attempt while processing is still active - checking status"
        );
        // Check if we're in a state where we should allow hiding anyway
        if (this.textProcessingComplete) {
          console.log(
            "🖼️ Text processing is complete, allowing popup to close even though isProcessing is true"
          );
          ...
          if (
            this.finalProcessingComplete &&
            (this.finalImageUrl || this.stylizedImageUrl)
          ) {
            this.showResultPopup(this.finalImageUrl || this.stylizedImageUrl);
          }
        }
  ...
  showResultPopup(imageUrl) {
    console.log("RESULT POPUP: Delegating to ResultPopupManager");
    ...
    if (window.resultPopupManager) {
      ...
      console.error(
        "ResultPopupManager not found, trying to load it dynamically"
      );

      // Try to load the ResultPopupManager dynamically
      ...
      script.onload = () => {
        ...
        if (window.resultPopupManager) {
          ...
          if (ResultPopupManager) {
            ...
            console.error("Could not load ResultPopupManager");
          }
        }
      };
  ...
  /**
   * Apply the selected aspect ratio to the result image - delegated to ResultPopupManager
   */
  ...
  /**
   * Adjust shadow to image - delegated to ResultPopupManager
   */
  adjustShadowToImage(image, shadow) {
    if (window.resultPopupManager) {
      window.resultPopupManager.adjustShadowToImage(image, shadow);
    }
  }
  ...
  applyFinalProcessing() {
    ...
    if (
      !this.cropCoordinates ||
      !this.cropCoordinates.width ||
      !this.cropCoordinates.height
    ) {
      console.warn(
        "FINAL PROCESSING: Invalid crop coordinates - will use default centered crop"
      );
    ...
    if (isRailwayUrl) {
      ...
      this.textInfo =
        this.textFields && (this.textFields.text || this.textFields.text2)
          ? {
              ...
              positionPercentage:
                this.textFields.positionPercentage ||
                (this.textFields.position === "top"
                  ? 10
              ...
              relativeCoordinates:
                this.textFields && this.textFields.relativeCoordinates
                  ? this.textFields.relativeCoordinates
                  : null,
            }
          : null;
    ...
    img.onload = () => {
      console.log(
        "FINAL PROCESSING: Stylized image loaded successfully, dimensions:",
        img.width,
        "x",
        img.height
      );
      ...
      console.log(
        "FINAL PROCESSING: Canvas dimensions set to:",
        finalWidth,
        "x",
        finalHeight
      );
      ...
      if (
        ...
        console.log(
          "FINAL PROCESSING: Applying crop coordinates:",
          JSON.stringify(this.cropCoordinates)
        );
        ...
        try {
          ...
          const originalHeight =
            this.cropCoordinates.originalImageHeight || 600;
          ...
          console.log(
            "FINAL PROCESSING: Original image dimensions:",
            originalWidth,
            "x",
            originalHeight
          );
          console.log(
            "FINAL PROCESSING: Stylized image dimensions:",
            img.width,
            "x",
            img.height
          );
          console.log(
            "FINAL PROCESSING: Scale factors from original to stylized - X:",
            originalToStylizedScaleX,
            "Y:",
            originalToStylizedScaleY
          );

          // Adjust crop coordinates to account for different sizes between original and stylized
          const adjustedCropX =
            this.cropCoordinates.x * originalToStylizedScaleX;
          const adjustedCropY =
            this.cropCoordinates.y * originalToStylizedScaleY;
          const adjustedCropWidth =
            this.cropCoordinates.width * originalToStylizedScaleX;
          const adjustedCropHeight =
            this.cropCoordinates.height * originalToStylizedScaleY;
          ...
          console.log(
            "FINAL PROCESSING: Original crop coordinates:",
            JSON.stringify(this.cropCoordinates)
          );
          console.log(
            "FINAL PROCESSING: Adjusted crop coordinates:",
            JSON.stringify(this.adjustedCropDimensions)
          );
          ...
          console.log(
            "FINAL PROCESSING: Scale factors from crop to canvas - X:",
            cropToCanvasScaleX,
            "Y:",
            cropToCanvasScaleY
          );
          ...
          console.log(
            "FINAL PROCESSING: Applied offsets - X:",
            offsetX,
            "Y:",
            offsetY
          );
          ...
          ctx.drawImage(
            img, // Source image
            0,
            0, // Source position (0,0)
            img.width,
            img.height, // Source dimensions (full image)
            offsetX,
            offsetY, // Destination position (with offsets to show cropped area)
            img.width * cropToCanvasScaleX, // Destination width (scaled)
            img.height * cropToCanvasScaleY // Destination height (scaled)
          );

          console.log(
            "FINAL PROCESSING: Image successfully drawn with crop applied"
          );
        } catch (error) {
          console.error(
            "FINAL PROCESSING: Error drawing image with crop:",
            error
          );
          // Fallback to drawing without crop
          ctx.drawImage(img, 0, 0, finalWidth, finalHeight);
          console.log(
            "FINAL PROCESSING: Fallback - Image drawn without crop due to error"
          );
        }
      } else {
        // No cropping - just center the image
        console.log(
          "FINAL PROCESSING: No crop coordinates available, centering image"
        );
        ctx.drawImage(img, 0, 0, finalWidth, finalHeight);
      }
      ...
      if (this.textFields && (this.textFields.text || this.textFields.text2)) {
        ...
        console.log(
          "FINAL PROCESSING: Adding Name1 & Name2 text to image:",
          name1,
          name2
        );
        if (subtitle) {
        ...
        const textLength =
          name1.length + name2.length + (name1 && name2 ? 3 : 0);
        ...
        const calculatedFontSize = Math.floor(
          usableWidth / (textLength * 0.45)
        ); // Reduced multiplier from 0.55 to 0.45 for bigger text
        ...
        const totalTextWidth =
          firstPartWidth + ampersandWidth + secondPartWidth;
        ...
        scaledFontSize = Math.max(
          minFontSize,
          Math.min(maxFontSize, scaledFontSize)
        );

        console.log(
          "FINAL PROCESSING: Font size precision scaling - initial:",
          calculatedFontSize,
          "measured width:",
          totalTextWidth,
          "target width:",
        ...
        console.log(
          "FINAL PROCESSING: Text scaling - textLength:",
          textLength,
        ...
        console.log(
          "FINAL PROCESSING: Dynamic letter spacing:",
          letterSpacingEm + "em",
          "for font size:",
          scaledFontSize
        );
        ...
        if (testTextWidth > maxFontWidth) {
          ...
          scaledFontSize = Math.floor(
            scaledFontSize * (maxFontWidth / testTextWidth)
          ); // Removed safety margin
          console.log(
            "FINAL PROCESSING: Adjusting font size to",
            scaledFontSize,
            "because text width",
            testTextWidth,
            "exceeds usable width",
            maxFontWidth
          );
          ctx.font = `900 ${scaledFontSize}px 'Montserrat', 'Arial Black', sans-serif`;
        }
        ...
        if (this.textFields.relativeCoordinates) {
          ...
          const croppedWidth =
            this.textFields.relativeCoordinates.croppedImageWidth ||
            this.cropCoordinates.width;
          const croppedHeight =
            this.textFields.relativeCoordinates.croppedImageHeight ||
            this.cropCoordinates.height;
          ...
          const yPercent =
            this.textFields.relativeCoordinates.y / croppedHeight;
          ...
          console.log(
            "FINAL PROCESSING: Text positioned using relative coordinates - X:",
            x,
            "Y:",
            y
          );
          console.log(
            "FINAL PROCESSING: Text position calculation - xPercent:",
            xPercent,
            "yPercent:",
            yPercent
          );
          ...
          console.log(
            "FINAL PROCESSING: Text positioned using standard method - X:",
            x,
            "Y:",
            y
          );
        ...
        if (lines.length > 1) {
          ...
          console.log(
            "FINAL PROCESSING: Drawing multi-line text, total height:",
            totalTextHeight
          );
          ...
          if (name1 && name2) {
            ...
            const totalWidth =
              totalName1Width + ampersandWidth + totalName2Width;
            ...
            if (totalWidth > canvas.width * 0.8) {
              ...
              const newTotalWidth =
                totalName1Width + newAmpersandWidth + totalName2Width;

              console.log(
                "FINAL PROCESSING: Adjusted font size to",
                scaledFontSize,
                "for custom letter spacing, new total width:",
                newTotalWidth
              );
            }
            ...
            console.log(
              'FINAL PROCESSING: Drew "Name 1 & Name 2" format with adjusted letter spacing'
            );
          } else if (name1) {
            // Draw just name1
        ...
        if (subtitle) {
          ...
          const subtitleFontSize = Math.max(
            Math.floor(scaledFontSize * 0.35),
            16
          );
          ...
          if (subtitleWidth > maxSubtitleWidth) {
            console.log(
              "FINAL PROCESSING: Subtitle too long, wrapping or truncating"
            );
            ...
            if (lines.length > 2) {
              console.log(
                "FINAL PROCESSING: Subtitle has too many lines, truncating to 2"
              );
            ...
            console.log(
              "FINAL PROCESSING: Wrapped subtitle into",
              lines.length,
              "lines"
            );
          } else {
            ...
            console.log(
              "FINAL PROCESSING: Subtitle added as single line at Y:",
              subtitleY
            );
          }
        }
      ...
      console.log(
        "FINAL PROCESSING: Final image created, updating product gallery"
      );
      ...
      document.dispatchEvent(
        new CustomEvent("pixar-final-processing-complete", {
          detail: { imageUrl: this.finalImageUrl },
        })
      );
      console.log(
        "FINAL PROCESSING: Dispatched pixar-final-processing-complete event"
      );
    ...
    img.onerror = (error) => {
      ...
      console.warn(
        "FINAL PROCESSING: Fallback - Displaying original stylized image without crop modifications"
      );
      ...
      console.log(
        "FINAL PROCESSING: Dispatched pixar-final-processing-complete event (with error)"
      );
    ...
    console.log(
      "FINAL PROCESSING: Loading stylized image:",
      this.stylizedImageUrl
    );
  ...
  setupDefaultCrop() {
    ...
    console.log(
      "FINAL PROCESSING: Created default centered crop coordinates:",
      this.cropCoordinates
    );
  }
  ...
  updateProductImage(imageUrl) {
    console.log(
      "PRODUCT IMAGE: Updating all product gallery images with new image"
    );

    try {
      // Find the main product image (if already transformed, use that one first)
      const mainImage =
        document.querySelector(".pixar-transformed-image") ||
        this.findMainProductImage();

      if (mainImage) {
        // Simply update the source of the image - don't apply any cropping
        console.log(
          "PRODUCT IMAGE: Using direct DOM updates with:",
        ...
        const isAuroraTheme =
          !!document.querySelector(".product-gallery__media") ||
          !!document.querySelector(".product-gallery__media-list");

        if (isAuroraTheme) {
          console.log(
            "PRODUCT IMAGE: Detected Aurora theme, applying special handling"
          );

          // Find the gallery media container - this could be several levels up
          const mediaContainer =
            mainImage.closest(".product-gallery__media") ||
          ...
          if (mediaContainer) {
            ...
            const mediaListItem = mediaContainer.closest(
            ...
            const mediaWrapper = mainImage.closest(
        ...
        console.log(
          `PRODUCT IMAGE: Updated ${
            1 + additionalImagesUpdated
          } product images with new design`
        );
    ...
    try {
      ...
      for (const selector of thumbnailSelectors) {
        ...
        if (activeThumbnailImg) {
          ...
          const container = activeThumbnailImg.closest(
  ...
  validateImageDisplay(image) {
    ...
    const currentRatio =
      parseFloat(image.clientWidth) / parseFloat(image.clientHeight);
    const targetRatio = 3 / 4;

    // Check if the aspect ratio is significantly different from our target
    if (Math.abs(currentRatio - targetRatio) > 0.1) {
      console.warn(
        `PRODUCT IMAGE: Aspect ratio validation failed - current: ${currentRatio.toFixed(
          2
      ...
      const isInWrapper =
        image.parentElement &&
        image.parentElement.classList.contains("pixar-transform-container");
      if (!isInWrapper) {
  ...
  createReplacementImage(imageUrl) {
    ...
    if (container) {
      ...
      console.error(
        "PRODUCT IMAGE: Could not find any suitable container for replacement image"
      );
    }
  }
  ...
  applyRailwayCropStyling() {
    console.log(
      "RAILWAY PROCESSING: Applying Railway image with CSS-based cropping"
    );
    ...
    try {
      ...
      if (!mainImage) {
        console.log(
          "RAILWAY PROCESSING: Main product image not found initially, trying again after delay"
        );
        ...
        setTimeout(() => {
          ...
          if (retryMainImage) {
            console.log(
              "RAILWAY PROCESSING: Found main product image on retry:",
              retryMainImage.src
            );
            this.processCropForMainImage(retryMainImage);
          } else {
            console.log(
              "RAILWAY PROCESSING: Still could not find main product image, proceeding with fallback"
            );
      ...
      console.error(
        "RAILWAY PROCESSING: Error applying Railway image crop:",
        error
      );
  ...
  processCropForMainImage(mainImage) {
    ...
    img.onload = () => {
      console.log(
        "RAILWAY PROCESSING: Original image loaded",
        img.width,
        "x",
        img.height
      );
      ...
      if (this.cropCoordinates) {
        ...
        console.log(
          `RAILWAY PROCESSING: Original crop from ${crop.originalImageWidth}x${crop.originalImageHeight} to current ${this.originalImageWidth}x${this.originalImageHeight}`
        );
        console.log(
          `RAILWAY PROCESSING: Scaling factors - X: ${scaleX}, Y: ${scaleY}`
        );
        ...
        console.log(
          `RAILWAY PROCESSING: Adjusted crop coordinates: X: ${this.croppedImageX}, Y: ${this.croppedImageY}, Width: ${this.croppedImageWidth}, Height: ${this.croppedImageHeight}`
        );
        ...
        if (Math.abs(currentRatio - targetRatio) > 0.01) {
          ...
          console.log(
            `RAILWAY PROCESSING: Current aspect ratio ${currentRatio.toFixed(
              2
            )} doesn't match target ${targetRatio.toFixed(2)}, adjusting`
          );
          ...
          console.log(
            `RAILWAY PROCESSING: Adjusted to 3:4 ratio - Width: ${this.croppedImageWidth}, Height: ${this.croppedImageHeight}`
          );
        }
        ...
        console.log(
          "RAILWAY PROCESSING: No crop coordinates available, using full image"
        );

        // Set default crop to full image with 3:4 aspect ratio
        const fullImageRatio =
        ...
        if (fullImageRatio > targetRatio) {
          ...
          this.croppedImageWidth = Math.round(
            this.originalImageHeight * targetRatio
          );
          this.croppedImageX = Math.round(
            (this.originalImageWidth - this.croppedImageWidth) / 2
          );
        } else {
          ...
          this.croppedImageHeight = Math.round(
            this.originalImageWidth / targetRatio
          );
          this.croppedImageY = Math.round(
            (this.originalImageHeight - this.croppedImageHeight) / 2
          );
        }

        console.log(
          `RAILWAY PROCESSING: Set default crop to X: ${this.croppedImageX}, Y: ${this.croppedImageY}, Width: ${this.croppedImageWidth}, Height: ${this.croppedImageHeight}`
        );
      ...
      this.getCroppedImageFromDOM()
        .then((croppedImage) => {
          if (croppedImage) {
            console.log(
              "RAILWAY PROCESSING: Successfully captured cropped image"
            );

            // If no text to add, we're done - update product image with the cropped image
            if (!this.textInfo) {
              console.log(
                "RAILWAY PROCESSING: No text to add, updating image and dispatching final event"
              );
              ...
              console.log(
                "RAILWAY PROCESSING: Adding Name1 & Name2 text to image"
              );
            ...
            console.log(
              "RAILWAY PROCESSING: Could not capture cropped image, using original image"
            );
        ...
        .catch((error) => {
          console.error(
            "RAILWAY PROCESSING: Error in processing pipeline:",
            error
          );
          this.handleMainImageNotFound();
        });
  ...
  handleMainImageNotFound() {
    console.error(
      "RAILWAY PROCESSING: Could not find or load main product image"
    );

    // We can still try to process the image for text if needed
    if (this.textInfo) {
      console.log(
        "RAILWAY PROCESSING: Attempting to add text to original image without cropping"
      );
      ...
      if (!this.cropCoordinates || !this.croppedImageWidth) {
        ...
        console.log(
          `RAILWAY PROCESSING: Set default fallback crop - W:${this.croppedImageWidth}, H:${this.croppedImageHeight}`
        );
      }
  ...
  loadDirectImageFallback() {
    return new Promise((resolve, reject) => {
      ...
      const baseImageUrl =
        "https://cartoonique.com/cdn/shop/files/f8336544-577a-4aff-9e06-c9df35868714.webp";

      console.log(
        "RAILWAY PROCESSING: Loading fallback image from:",
        baseImageUrl
      );
      ...
      baseImg.onload = () => {
        try {
          console.log(
            "RAILWAY PROCESSING: Fallback base image loaded, dimensions:",
            baseImg.width,
            "x",
            baseImg.height
          );
          ...
          console.log(
            "RAILWAY PROCESSING: Successfully captured fallback image"
          );
          resolve(dataURL);
        } catch (err) {
          console.error(
            "RAILWAY PROCESSING: Error processing fallback image:",
            err
          );
          reject(err);
        }
      ...
      baseImg.onerror = (err) => {
        ...
        alternateImg.onerror = (altErr) => {
          console.error(
            "RAILWAY PROCESSING: Error loading alternate fallback image:",
            altErr
          );
          reject(altErr);
        };
  ...
  applyCropStylingToImages() {
    return new Promise((resolve) => {
      ...
      console.log(
        "CROP STYLING: Using direct image capture instead of CSS styling"
      );
      resolve();
    });
  }
      ...
      try {
        ...
        const mainImg =
          document.querySelector(
            ".product-gallery__media.snap-center.is-selected img"
          ) ||
        ...
        if (!mainImg) {
          console.error(
            "RAILWAY PROCESSING: Could not find main product image"
          );

          // Log all available image elements on the page to help with debugging
          const allImages = document.querySelectorAll("img");
          console.log(
            "RAILWAY PROCESSING: Available images on page:",
            allImages.length
          );
        ...
        console.log(
          "RAILWAY PROCESSING: Found main product image:",
          mainImg.src.substring(0, 100) + "..."
        );
        ...
        sourceImg.onload = () => {
          try {
            ...
            const cropWidth =
              this.cropCoordinates.width * scaleX * (1 + padding * 2);
            const cropHeight =
              this.cropCoordinates.height * scaleY * (1 + padding * 2);

            // Center the crop with the padding
            const cropX = Math.max(
              0,
              this.cropCoordinates.x * scaleX -
                this.cropCoordinates.width * scaleX * padding
            );
            const cropY = Math.max(
              0,
              this.cropCoordinates.y * scaleY -
                this.cropCoordinates.height * scaleY * padding
            );

            // Make sure we don't exceed the original image dimensions
            const adjustedCropWidth = Math.min(
              cropWidth,
              sourceImg.width - cropX
            );
            const adjustedCropHeight = Math.min(
              cropHeight,
              sourceImg.height - cropY
            );
            ...
            ctx.drawImage(
              ...
              cropY,
              adjustedCropWidth,
              adjustedCropHeight, // Source rectangle
              drawX,
              drawY,
              adjustedCropWidth,
              adjustedCropHeight // Destination rectangle
            );
            ...
            console.error(
              "RAILWAY PROCESSING: Error capturing cropped image:",
              err
            );
            reject(err);
          }
  ...
  addTextToRailwayImage(imageUrl) {
    console.log(
      "RAILWAY TEXT: Image loaded, dimensions:",
    ...
    // Add text with names
    console.log(
      "RAILWAY TEXT: Adding text with names:",
      this.textInfo.text,
      this.textInfo.text2
    );
    ...
    img.onload = () => {
      try {
        ...
        // Dispatch event to signal final processing is complete
        console.log(
          "FINAL PROCESSING: Dispatched pixar-final-processing-complete event after text processing"
        );
        const finalEvent = new CustomEvent("pixar-final-processing-complete", {
          detail: {
            imageUrl: dataURL,
  ...
  pollRailwayJobStatus(jobId, attempt = 1) {
    console.log(
      `🖼️ Polling Railway job status for job ${jobId}, attempt ${attempt}`
    );
    ...
    if (
      window.railwayJobsStatus[jobId] === "COMPLETED" ||
      window.railwayJobsStatus[jobId] === "FAILED"
    ) {
      console.log(
        `🖼️ Job ${jobId} already has final status: ${window.railwayJobsStatus[jobId]}, skipping poll`
      );
      return;
    }

    // Track that we're polling this job
    window.railwayJobsStatus[jobId] =
      window.railwayJobsStatus[jobId] || "PENDING";

    // Implement reasonable timeout/limits
    if (attempt > 30) {
      console.log(
        `🖼️ Giving up on polling job ${jobId} after ${attempt} attempts`
      );
      window.railwayJobsStatus[jobId] = "FAILED";
      return;
    }
    ...
    fetch(
      ...
      .then((data) => {
        ...
        if (data.status && data.status.toUpperCase() === "COMPLETED") {
          ...
          if (imageUrl) {
            ...
            this.stylizedImageUrl = imageUrl;
            this.stylizednonImageUrl =
              data.processedImageUrl || data.watermarkedImageUrlToShow;
            this.transformationComplete = true;
          ...
          const backoffTime = Math.min(
            1000 * Math.pow(1.5, Math.min(attempt - 1, 10)),
            10000
          );
          console.log(`🖼️ Continuing to poll job ${jobId} in ${backoffTime}ms`);
          setTimeout(
            () => this.pollRailwayJobStatus(jobId, attempt + 1),
            backoffTime
          );
        }
      })
      .catch((error) => {
        ...
        // Retry with backoff
        const backoffTime = Math.min(
          2000 * Math.pow(1.5, Math.min(attempt - 1, 8)),
          15000
        );
        setTimeout(
          () => this.pollRailwayJobStatus(jobId, attempt + 1),
  ...
  getRailwayStats() {
    ...
    if (window.railwayApiCallsInProgress) {
      stats.totalTrackedFiles = Object.keys(
  ...
  sendImageToRailway(file, options = {}) {
    ...
    if (window.railwayApiCallsInProgress[fileIdentifier]) {
      console.log(
        `🖼️ Already processing file ${file.name}, skipping duplicate request`
      );
      ...
      reader.onload = (e) => {
        ...
        fetch(
          ...
          .then((data) => {
            ...
            if (jobId) {
              console.log(
                `🖼️ Successfully received jobId: ${jobId} for file ${file.name}`
              );
  ...
  drawTextOnCanvas(ctx, width, height) {
    ...
    console.log(
      "RAILWAY TEXT: Font size precision scaling - initial:",
      calculatedFontSize,
      "measured width:",
      totalTextWidth,
      "target width:",
    ...
    console.log(
      "RAILWAY TEXT: Font size calculation - usableWidth:",
      usableWidth,
      "textLength:",
      textLength
    );
    console.log(
      "RAILWAY TEXT: Calculated font size:",
      scaledFontSize,
      "from raw calculation:",
      calculatedFontSize
    );
    ...
    const letterSpacingEm = Math.max(0.01, 0.04 - scaledFontSize / 2000); // Reduced letter spacing for taller text
    console.log(
      "RAILWAY TEXT: Dynamic letter spacing:",
      letterSpacingEm + "em",
      "for font size:",
      scaledFontSize
    );
    ...
    if (testTextWidth > maxFontWidth) {
      ...
      console.log(
        "RAILWAY TEXT: Adjusting font size to",
        scaledFontSize,
        "because text width",
        testTextWidth,
        "exceeds usable width",
  ...
  findMainProductImage() {
    ...
    try {
      ...
      mainImage =
        document.querySelector(
          ".product__media img:not(.product__thumbnail)"
        ) ||
        document.querySelector(
          ".product-media-modal__content img.product__media-item"
        );

      // Empire theme pattern
      if (!mainImage) {
        mainImage =
          document.querySelector(".product-single__photo:not(.hide) img") ||
          document.querySelector(
            '.photoswipe__image:not([aria-hidden="true"]) img'
          );
      }
      ...
      if (!mainImage) {
        mainImage =
          document.querySelector(".product-single__media--featured img") ||
      ...
      if (!mainImage) {
        ...
        if (galleryContainer) {
          mainImage = galleryContainer.querySelector(
            "img:not(.product__thumbnail)"
          );
        }
      }
  ...
  getCroppedImageFromDOM() {
    try {
      ...
      console.log(
        `🖼️ Creating crop canvas with dimensions ${canvas.width}x${canvas.height}`
      );
      console.log(
        `🖼️ Using crop coordinates X:${this.croppedImageX}, Y:${this.croppedImageY}, W:${this.croppedImageWidth}, H:${this.croppedImageHeight}`
      );
      ...
      return new Promise((resolve, reject) => {
        img.onload = () => {
          try {
            console.log(
              `🖼️ Source image loaded with dimensions ${img.width}x${img.height}`
            );
            ...
            try {
              ...
              const dataURL = canvas.toDataURL("image/png", 0.95);
              console.log(
                "🖼️ Successfully captured image with fallback approach"
              );
              resolve(dataURL);
            } catch (fallbackError) {
  ...
  applyCropStylingToImages() {
    try {
      ...
      if (mainImage) {
        ...
        console.log(
          `🖼️ Found ${imagesToTransform.length} additional product images to apply styling to`
        );
        ...
        console.log(
          `🖼️ Applied crop styling to ${
            imagesToTransform.length + 1
          } product images (including main image)`
        );
      } else {
        console.warn(
          "🖼️ Main product image not found, cannot apply crop styling"
        );
      }
    } catch (error) {
  ...
  wrapImageInContainer(img) {
    ...
    if (
      img.parentElement &&
      !img.parentElement.classList.contains("pixar-transform-container")
    ) {
      ...
      if (needsContainer) {
        console.log(
          `🖼️ Creating wrapper container for image ${img.src.substring(
            0,
            50
          )}...`
        );
  ...
  setupCartImageReplacementWithFallbacks() {
    console.log(
      "Setting up cart image replacement with fallback storage handling"
    );
      ...
      (function() {
        ...
        try {
          ...
          if (navDataStr) {
            ...
            if (navigationData && navigationData.variantId && navigationData.imageUrl) {
              ...
              // Also store in window for other access methods
              window.cartoonique_navigationData = navigationData;
            }
          }
        } catch (e) {
        ...
        function replaceCartImages() {
          ...
          cartItems.forEach(item => {
            const variantId = item.dataset.variantId || item.dataset.id || item.dataset.itemVariantId || item.dataset.cartItemId;
            if (variantId) {
          ...
          cartImages.forEach(img => {
            ...
            function findVariantIdForImage(img) {
              ...
              for (let depth = 0; depth < 10; depth++) {
                ...
                // Check surrounding elements by walking the DOM tree up
                element = element.parentElement;
                if (!element) break;
                checkedElements.add(element);
                
                // Check for data attributes
                const parentVariantId = element.dataset.variantId || 
                                      element.dataset.id || 
                                      element.dataset.variant || 
                                      element.dataset.itemId ||
                                      element.dataset.cartItemId;
                
                if (parentVariantId) return parentVariantId;
              ...
              if (variantIds.size > 0) {
                return Array.from(variantIds)[0];
              }
              
              // Try navigation data as last resort
              if (navigationData && navigationData.variantId) {
                return navigationData.variantId;
              }
            ...
            if (foundVariantId) {
              ...
              if (navigationData && navigationData.imageUrl) {
                ...
                img.srcset = '';
                img.sizes = '';
                img.src = navigationData.imageUrl;
                img.dataset.processed = 'true';
                img.loading = 'eager';
                img.style.objectFit = 'contain';
                replacedCount++;
        ...
        if (window.location.pathname.includes('/cart')) {
          ...
          // And also run after DOM is fully loaded
          document.addEventListener('DOMContentLoaded', () => {
            replaceCartImages();
          });
        }
        
        // Run immediately for all cases
        replaceCartImages();
        
        // Run with progressive delays to catch late-loading cart elements
        ...
        if (typeof MutationObserver !== 'undefined') {
          const observer = new MutationObserver((mutations) => {
            // Only run if we see DOM changes that might include cart images
            let shouldRun = false;
          ...
          const cartContainers = [
            document.body, // Observe everything as a fallback
            document.querySelector('.cart'), 
            document.querySelector('#CartDrawer'),
            document.querySelector('.cart-notification-popup')
          ].filter(Boolean);
  ...
  setupCartImageReplacement() {
    console.log(
      "Redirecting to setupCartImageReplacementWithFallbacks for better storage handling"
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains("processedImages")) {
          const store = db.createObjectStore("processedImages", {
            keyPath: "variantId",
          });
  ...
  cleanupOldImages() {
    ...
    const transaction = this.imageDB.transaction(
      ["processedImages"],
      "readwrite"
    );
    ...
    request.onsuccess = (event) => {
      ...
      if (cursor) {
        ...
        if (imagesToDelete.length > 0) {
          console.log(
            `Cleaned up ${imagesToDelete.length} old images from IndexedDB`
          );
        }
      }
    };
  ...
  storeImageInIndexedDB(variantId, imageData) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        request.onerror = (event) => {
          console.error(
            "Error storing image in IndexedDB:",
            event.target.error
          );
          resolve(false);
        };
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        request.onsuccess = (event) => {
          ...
          if (result) {
            console.log(
              `Retrieved image for variant ${variantId} from IndexedDB`
            );
  ...
  storeImageSafely = async (variantId, imageUrl) => {
    try {
      ...
      if (this.imageDB) {
        indexedDBSuccess = await this.storeImageInIndexedDB(
          variantId,
          imageUrl
        );
      }
      ...
      try {
        ...
        cartImages[variantId] = indexedDBSuccess
          ? "INDEXED_DB:" + variantId
          : "MEMORY_IMAGE:" + variantId;
...
if (window.processImageWithRunPod && !window.originalProcessImageWithRunPod) {
  ...
  window.processImageWithRunPod = function (file, options = {}) {
    console.log(
      "🖼️ Image Processing Manager intercepted processImageWithRunPod call for file:",
      file.name
    );
    ...
    if (window.imageProcessingManager) {
      window.imageProcessingManager
        ...
        .then((result) => {
          if (result.alreadyProcessing) {
            console.log(
              "🖼️ File is already being processed by Railway, avoiding duplicate call"
            );
          } else {
            console.log(
              `🖼️ File sent to Railway via centralized method, job ID: ${result.jobId}`
            );
          }
        })
        .catch((error) => {
          console.error(
            "🖼️ Error sending to Railway via centralized method:",
            error
          );
          ...
          if (window.originalProcessImageWithRunPod) {
            console.log(
              "🖼️ Falling back to original processImageWithRunPod implementation"
            );
            try {
              window.originalProcessImageWithRunPod(file, options);
            } catch (err) {
              console.error(
                "🖼️ Error in original processImageWithRunPod:",
                err
              );
            }
          }
      ...
      console.log(
        "🖼️ ImageProcessingManager not available, using original implementation"
      );
      if (window.originalProcessImageWithRunPod) {
        try {
          window.originalProcessImageWithRunPod(file, options);
  ...
  console.log(
    "🖼️ processImageWithRunPod not found at initialization time, will check later"
  );

  // Try to set it up after a short delay to ensure it's available
  setTimeout(() => {
    if (
      window.processImageWithRunPod &&
      !window.originalProcessImageWithRunPod
    ) {
      console.log(
        "🖼️ Found processImageWithRunPod on retry, setting up interception"
      );
      window.originalProcessImageWithRunPod = window.processImageWithRunPod;
      window.processImageWithRunPod = function (file, options = {}) {
        console.log(
...
const originalHandleCropComplete =
  ImageProcessingManager.prototype.handleCropComplete;
if (originalHandleCropComplete) {
  ImageProcessingManager.prototype.handleCropComplete = function (
    croppedImage
  ) {
    ...
    if (
      ...
      this.sendImageToRailway(file, { isCropped: true })
        .then((result) => {
          if (result.alreadyProcessing) {
            ...
            console.log(
              `🖼️ Cropped image sent to Railway, job ID: ${result.jobId}`
            );
          }
        })
        .catch((error) => {
          ...
          if (typeof window.pixarComponent?.handleFileSelect === "function") {
            console.log(
              "🖼️ Falling back to pixarComponent.handleFileSelect with cropped image"
            );
      ...
      console.log(
        "🖼️ Cropping complete, but image was already sent to Railway earlier or processImageWithRunPod is available"
      );
    }
  };
