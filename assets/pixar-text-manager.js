...
if (typeof window.PixarTextManager === "undefined") {
  class PixarTextManager {
     ...
     * Get the text overlay component
     */
    async getPetTextOverlay() {
      if (this.textOverlay) {
        return this.textOverlay;
      }

      // Find existing component or create one
      let overlay = document.querySelector("pet-text-overlay");
      if (!overlay) {
        // Create the component
        overlay = document.createElement("pet-text-overlay");
        document.body.appendChild(overlay);
        this.log("Created text overlay component");
      }
    ...
    async showTextDialog(imageFile) {
      try {
        ...
        if (window?.isPetTemplate) {
          overlay = await this.getPetTextOverlay();
        } else {
          overlay = await this.getTextOverlay();
        }
        ...
        const imageUrl = window?.isPetTemplate
    ...
    async applyTextToImage(imageUrl) {
      try {
        ...
        console.error(
          "[PixarTextManager] Error applying text to image:",
          error
        );
