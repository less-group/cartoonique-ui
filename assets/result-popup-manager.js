...
class ResultPopupManager {
  ...
  /**
   * Initialize variant IDs based on template type and try to get from product data
   */
  initializeVariantIds() {
    ...
    if (productVariants && Object.keys(productVariants).length > 0) {
      console.log("Using variant IDs from page data:", productVariants);
      this.variantIds = productVariants;
    } else if (isPetTemplate) {
      console.log("Pet template detected - using hardcoded pet variant IDs");
      // Hardcoded variant IDs for pet products as fallback
      this.variantIds = {
        S: "54386981110108", // Pet product - Size S (20x30 cm)
        M: "54386981142876", // Pet product - Size M (30x40 cm)  
        L: "54386981175644", // Pet product - Size L (50x70 cm)
      };
    } else {
      console.log("Standard template detected - using hardcoded standard variant IDs");
      // Hardcoded variant IDs for standard products as fallback
      this.variantIds = {
        S: "54224438722908", // Standard product - Size S
        M: "54215893451100", // Standard product - Size M
        L: "54215893483868", // Standard product - Size L
      };
    }
    
    console.log("Final variant IDs:", this.variantIds);
  }

  /**
   * Try to extract variant IDs from the page's product data
   */
  getProductVariantsFromPage() {
    const variants = {};
    
    // Method 1: Check for product JSON in script tags
    try {
      ...
      if (productScript) {
        ...
        if (productData.variants) {
          productData.variants.forEach(variant => {
            // Map variant options to our size keys
            const sizeOption = variant.options?.find(opt => 
              opt === "20x30 cm" || opt === "30x40 cm" || opt === "50x70 cm" ||
              opt === "S" || opt === "M" || opt === "L"
            );
            
            if (sizeOption) {
              const sizeKey = this.mapSizeToKey(sizeOption);
              if (sizeKey) {
                variants[sizeKey] = variant.id.toString();
              }
            }
          });
        }
    ...
    // Method 2: Check for variant selectors in the form
    try {
      const variantInputs = document.querySelectorAll('input[name="id"][data-size], option[data-variant-id]');
      variantInputs.forEach(input => {
        const size = input.dataset.size || input.textContent?.trim();
        const variantId = input.value || input.dataset.variantId;
        if (size && variantId) {
          const sizeKey = this.mapSizeToKey(size);
          if (sizeKey) {
            variants[sizeKey] = variantId.toString();
          }
        }
      });
    } catch (e) {
      console.log("Could not extract variants from form:", e);
    }
    
    // Method 3: Check window.productVariants if available
    if (window.productVariants) {
      try {
        Object.entries(window.productVariants).forEach(([key, value]) => {
          const sizeKey = this.mapSizeToKey(key);
          if (sizeKey) {
            variants[sizeKey] = value.toString();
          }
        });
      } catch (e) {
        console.log("Could not use window.productVariants:", e);
      }
    }
  ...
  /**
   * Map various size formats to our standard S/M/L keys
   */
  mapSizeToKey(sizeString) {
    if (!sizeString) return null;
    
    const size = sizeString.toString().toUpperCase();
    ...
    // Size in cm
    if (size.includes("20X30") || size.includes("20 X 30")) return "S";
    if (size.includes("30X40") || size.includes("30 X 40")) return "M";
    if (size.includes("50X70") || size.includes("50 X 70")) return "L";
      ...
      resultPopup.innerHTML = `
        <div id="final-popup" style="position: relative; max-width: 600px; margin: 20px auto; padding: 20px; background: white; border-radius: 12px; box-shadow: 0 0 30px rgba(0,0,0,0.1); display: flex; flex-direction: column; height: calc(100vh - 40px); max-height: 800px;">
          ...
          <div style="flex-shrink: 0; margin-top: 5px;">
            ...
            <div style="display: flex; justify-content: center; gap: 10px; margin-top: 10px; margin-bottom: 10px; flex-wrap: nowrap;">
            ${
              window?.isPetTemplate
                ? '<button id="pixar-result-revert" style="background-color: #838383; color: white; padding: 12px 25px; font-size: 16px; font-weight: bold; border: none; border-radius: 8px; cursor: pointer; text-transform: uppercase; box-shadow: 0 4px 8px rgba(0,0,0,0.1); transition: background-color 0.2s ease-in-out; white-space: nowrap;">GO BACK</button>'
                : ""
            }
              <button id="pixar-result-continue" style="background-color: #4A7DBD; color: white; padding: 12px 25px; font-size: 16px; font-weight: bold; border: none; border-radius: 8px; cursor: pointer; text-transform: uppercase; box-shadow: 0 4px 8px rgba(0,0,0,0.1); transition: background-color 0.2s ease-in-out; white-space: nowrap;">ADD TO CART</button>
            </div>
          </div>
        </div>
      `;
  ...
  async redirectToCheckout(size) {
    ...
    console.log("Available variant IDs:", this.variantIds);
    ...
    if (!variantId) {
      ...
      console.error("Available sizes:", Object.keys(this.variantIds));
      alert("Error: Invalid product variant. Please refresh the page and try again.");
      return;
    }
    
    console.log("Using variant ID:", variantId, "for size:", size);
    ...
    fetch("/cart/add.js", {
      ...
      .then((response) => {
        if (!response.ok) {
          ...
          console.error("Size selected:", size);
          return response.text().then((text) => {
            ...
            try {
              ...
              if (errorData.description === "Cannot find variant") {
                console.error("Variant ID does not exist in Shopify:", variantId);
                alert(`Error: Product variant not found for size ${size}. Please contact support.`);
              }
            } catch (e) {
